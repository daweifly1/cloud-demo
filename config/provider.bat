...
cd  provider-web
call mvn clean package  -Ptest -Dserver.port=8871 -Dhostname=peer1 -DdefaultZone=http://peer1:8881/eureka/,http://peer2:8882/eureka/ -Dfinal.project.name=provider-web1

copy  %~dp0cloud-demo\provider-web\target\provider-web1-exec.jar    %~dp0tempp\provider-web1-exec.jar

call mvn clean package  -Ptest -Dserver.port=8872 -Dhostname=peer2 -DdefaultZone=http://peer1:8881/eureka/,http://peer2:8882/eureka/  -Dfinal.project.name=provider-web2
